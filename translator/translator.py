...
class MangaTranslator:
    ...
    def translate_batch(self, texts: list, method="google") -> list:
        ...
        if method == "google" and len(texts) > 1:
            max_workers = max(
                1,
                min(
                    int(os.environ.get("GOOGLE_TRANSLATE_MAX_WORKERS", "4")),
                    len(texts),
                ),
            )
            if max_workers == 1:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as ex:
                return list(ex.map(lambda t: self.translate(t, method), texts))
    ...
    def _translate_with_google(self, text):
        ...
        if not hasattr(self, "_google_tl_cache"):
            self._google_tl_cache = {}
        if key not in self._google_tl_cache:
            self._google_tl_cache[key] = GoogleTranslator(source=self.source, target=self.target)
        translator = self._google_tl_cache[key]
        try:
        ...
        # 检测 Google 可能将短文本错误地返回为原文的情况
        # 如果翻译结果完全由原文重复组成（如 "text text text"），返回原文
        base_len = len(text)
        if len(translated_text) >= base_len:
            for i in range(2, min(len(translated_text) // base_len + 1, 6)):
                if translated_text[:base_len * i] == text * i:
                    return text
        return translated_text
    ...
    def _load_nllb_model(self):
        ...
        Uses GPU if available (5-10x faster than CPU)."""
        if self._model_cache["nllb_model"] is None:
            print(f"Loading NLLB model on {self._nllb_device} (first time)...")
            ...
            self._model_cache["nllb_model"] = self._model_cache["nllb_model"].to(self._nllb_device)
            self._model_cache["nllb_model"].eval()
            print(f"NLLB model loaded on {self._nllb_device}!")

    def _translate_with_nllb(self, text):
        """
        Translate using Meta's NLLB-200 model.
        Supports 200+ languages, works offline, GPU-accelerated when available.
        """
        try:
            ...
            # src_lang as tokenizer param is thread-safe; no lock needed
    ...
    def _translate_batch_with_nllb(self, texts):
        ...
        Significantly faster for multiple bubbles. GPU-accelerated when available.
        """
        try:
            self._load_nllb_model()
            ...
            # src_lang as tokenizer param avoids lock contention; tensors move to GPU
            inputs = tokenizer(
                preprocessed_texts, return_tensors="pt",
                padding=True, truncation=True, src_lang=src_lang
            )
            inputs = {k: v.to(device) for k, v in inputs.items()}
