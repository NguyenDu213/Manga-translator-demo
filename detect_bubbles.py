...
def merge_yolo_with_black_bubbles(yolo_detections, image, enable_black_bubble):
    """
    Gộp kết quả YOLO với bubble đen (OpenCV), gắn cờ is_dark_bubble — cùng logic detect_bubbles().
    """
...
def detect_bubbles_batch_normal(model_path, images, enable_black_bubble=True):
    """
    Một forward YOLO cho nhiều ảnh (kích thước thường, không cắt chunk dài).
    Trả về list[detections] cùng thứ tự images.
    """
    if not images:
        return []
    ...
    print(f"YOLO batch inference: {len(images)} image(s)")
    yolo_results = model(images, verbose=False)
    out = []
    for i, r in enumerate(yolo_results):
        yolo_det = r.boxes.data.tolist()
        merged = merge_yolo_with_black_bubbles(yolo_det, images[i], enable_black_bubble)
        out.append(merged)
    return out
...
def detect_bubbles(model_path, image_input, enable_black_bubble=True):
    ...
    return merge_yolo_with_black_bubbles(yolo_detections, image, enable_black_bubble)
